...
public class GameFrame extends JFrame
{
    ...
    private boolean paused = true, newWave = true, addingBasicTower = false;
    ...
    private BlastAnimation shotAnimation1 = new BlastAnimation(),shotAnimation2 = new BlastAnimation();
    private GameView playArea;
    private int firstFiringTower=-1, secondFiringTower=-1, wave = 1,lastSize=0,delay2=0, pause = 0;
    private final int PLAY_SIZE_X = 40, PLAY_SIZE_Y=800, BUTTON_PANEL_X = 300, BUTTON_PANEL_Y=300, WINDOW_SIZE_X=1030,WINDOW_SIZE_Y=700;
    private final int X_BORDER_SIZE = 8, Y_BORDER_SIZE = 30,TILE_SIZE;
    ...
    GameFrame(int level, int wave){
        ...
        ActionListener addTowerListener = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                switch (buttonPressed) {
                    ...
                    case "Basic":
                        ...
                        try
                        {
                            BufferedImage buffered = ImageIO.read(new File(path+ "TowerDown.png"));
                            Cursor cursor = Toolkit.getDefaultToolkit().createCustomCursor(buffered,new Point(0,0),"tower");
                            setCursor(cursor);
        ...
        buttons.addActionListener(addTowerListener);
        ...
        this.addMouseListener(new MouseAdapter() {
            ...
            {
                //compensate for the frame border
                int xLoc = arg0.getX()-X_BORDER_SIZE;
                int yLoc = arg0.getY()-Y_BORDER_SIZE;
                ...
                if(tempTower.getCost()<=myBase.getMoney()) {
                    if(gameMap.canAdd(xLoc,yLoc)) {
                        tempTower.setLocation(xLoc-roundedX, yLoc-roundedY,TILE_SIZE);
            ...
            public void run() {
                ...
                mobTimer = new Timer(1000, new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        ...
                        testMob.setLocation(gameMap.getStartX(), gameMap.getStartY());
    ...
    public void gameUpdate() {
        ...
        if(monsters.size()>0&&!paused) {
            for(int monsterCount=0;monsterCount<monsters.size();monsterCount++) {
                ...
                //Check to see if the monster has reached the base. If yes damage the base and remove the monster
                if(monsters.get(monsterCount).getXLocation()==myBase.getYLocation()&&monsters.get(monsterCount).getYLocation()==myBase.getXLocation()) {
                    myBase.takeDamage(monsters.get(monsterCount).getDamage());
                    monsters.remove(monsterCount);
                }else {
                    //move the monsters
                    monsters.get(monsterCount).move(tile,findThreshold(xCoord,yCoord,tile));
                    //process each tower(check for in range monsters, fire.
                    for(int i=0;i<towers.size();i++) {
                        boolean foundTarget = false;
                        //Cycle through the monster list to see if any are in range. The first monster that is in range gets shot
                        for(int j=0; j<monsters.size() && !foundTarget;j++) {
                            Tower currentTower = towers.get(i);
                            Enemy currentEnemy = monsters.get(j);
                            if(currentTower.inRange(currentEnemy.getXLocation(),currentEnemy.getYLocation())){
                                //monsters.get(j).takeDamage(currentTower.getDamage());
                                currentEnemy.takeDamage(currentTower.getDamage());
                                foundTarget=true;
                                int towerX=currentTower.getCenterX(),towerY=currentTower.getCenterY(),monsterX=currentEnemy.getCenterX(), monsterY=currentEnemy.getCenterY();
                                currentTower.setImage(getAngle(towerX, towerY,monsterX, monsterY),true);
                                shots.add(new BlastAnimation(monsterX,monsterY,towerX,towerY));
    ...
    /**
     * This method calculates the angle of the monster with respect to the tower. It uses a vertical line through the the center of the tower.
     * @param towerX the xCoordinate of the tower
     * @param towerY the yCoordinate of the tower
     * @param monsterX the xCoordinate of the monster
     * @param monsterY the yCoordinate of the monster
     * @return the angle that was calculated with respect to the vertical y axis visually above the center of the tower(smaller y)
     */
    private double getAngle(int towerX, int towerY, int monsterX, int monsterY) {
        /* The towerY-10 is just an arbitrary offset to a point that is in vertical alignment with the tower */
        double d2 = Math.sqrt(Math.pow(towerX-towerX,2)+Math.pow(towerY-(towerY-10),2));
        double d3 = Math.sqrt(Math.pow(towerX-monsterX,2)+Math.pow(towerY-(monsterY),2));
        double d1 = Math.sqrt(Math.pow(towerX-monsterX,2)+Math.pow((towerY-10)-(monsterY),2));

        int normalizeAngle;
        double angle = Math.acos((Math.pow(d1,2)+Math.pow(d2,2)-Math.pow(d3,2))/(2*d1*d2))*180/Math.PI;
        if(monsterX>towerX) {
            angle=180-angle;
    ...
    /**
     * The game maps are based on a tile system. This method finds the tile that a coordinate belongs to.
     * @param xCoord xcoord of the object
     * @param yCoord ycoord of the object
    ...
    /**
     * Thresholds are used to determine when a monster needs to adjust it's path. Certain types of tiles indicate that a mob will
     * need to change direction to N,E,S, or W. When a mob enters a tile like this, it will change direction once it crosses the
     * threshold. This is used to keep the sprites aligned with the map tiles.
     * @param xCoord of the Mob
     * @param yCoord of the Mob
    ...
    private int findThreshold(int xCoord, int yCoord, int tile) {
        ...
        switch (tile%10) {
            case 1: 
            case 3: threshold = (column*TILE_SIZE);
                    break;
            case 2:
            case 4: threshold = (row*TILE_SIZE);
                    break;
        }
